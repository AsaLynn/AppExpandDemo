 ...
 * datepicker
 * date.picker
 * Created by liuwan onCouponConsumeRecordList 2016/9/28.
 */
public class CustomDatePicker {
	...
	public CustomDatePicker(Context context, ResultHandler resultHandler, String startDate, String endDate) {
		if (isValidDate(startDate, "yyyy-MM-dd HH:mm") && isValidDate(endDate, "yyyy-MM-dd HH:mm")) {
			...
			try {
				startCalendar.setTime(sdf.parse(startDate));// 2018.2.28
	...
	private void initTimer() {
		...
		if (spanYear) {
			...
			for (int i = startMonth; i <= endMonth; i++) {// 1,2
				month.add(formatTimeUnit(i));
			}
			for (int i = startDay; i <= startCalendar.getActualMaximum(Calendar.DAY_OF_MONTH); i++) {
				day.add(formatTimeUnit(i));
	...
	public void isTimeShow(boolean show) {
		this.isTimeShow = show;
	}
	...
	public void setSelectedTime(String time) {
		if (canAccess) {
			...
			if (selectedYear == startYear) {
				for (int i = startMonth; i <= MAX_MONTH; i++) {
					if (endCalendar.get(Calendar.MONTH) + 1 < startMonth) {// 修改跨年时月份初始化错误问题
						month.add(formatTimeUnit(i));
					} else {
						if (i <= endCalendar.get(Calendar.MONTH) + 1)
							month.add(formatTimeUnit(i));
					}
				}
			} else if (selectedYear == endYear) {
			...
			// int selectedMonth = selectedCalender.get(Calendar.MONTH) + 1;
			int selectedMonth = Integer.parseInt(dateStr[1]);
			if (isTimeShow) {
//				day.clear();
//				for (int i = 1; i <= Integer.parseInt(dateStr[2]); i++) {
//					day.add(formatTimeUnit(i));
//				}
				day.clear();
				if (selectedYear == startYear && selectedMonth == startMonth) {
					if(startMonth != endMonth){//如果跨月
						for (int i = startDay; i <= selectedCalender.getActualMaximum(Calendar.DAY_OF_MONTH); i++) {
							day.add(formatTimeUnit(i));
						}
					}else{
				...
				} else if (selectedYear == endYear && selectedMonth == endMonth) {//跨年
					for (int i = 1; i <= endDay; i++) {
						day.add(formatTimeUnit(i));
					}
				} else {
